...
export class PaymentsComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.paymentsService
      ...
      .subscribe(
        (res: HttpResponse<IPayments[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IPayments): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInPayments(): void {
    this.eventSubscriber = this.eventManager.subscribe('paymentsListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IPayments[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
